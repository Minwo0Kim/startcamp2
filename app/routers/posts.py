...
from ..schemas import C_Post, C_PostResponse, AllPostDict, R_AllPostResponse, R_PostResponse, U_Post, D_Post
...
#전체 게시글 목록 조회
@posts_router.get("", response_model = R_AllPostResponse)
...
async def get_post(post_id : int, db : Session = Depends(get_db)):
    ...
    if post is not None:
        ...
        raise HTTPException(status_code=404, detail = "삭제되었거나, 존재하지 않는 게시물입니다.")
...
async def delete_post(post_id : int, del_post : D_Post, db : Session = Depends(get_db)):
    ...
    if post is not None:
        ...
        raise HTTPException(status_code=404, detail = "이미 삭제되었거나, 존재하지 않는 게시물입니다.")
